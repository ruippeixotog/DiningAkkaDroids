...
class Chopstick(name: String) extends Actor with FSM[ChopstickState, TakenBy] {
  ...
  // When a chopstick is available, it can be taken by some hakker
  when(Available) {
    case Event(Take, _) =>
      goto(Taken) using TakenBy(Some(sender)) replying Taken(self)
  }

  // When a chopstick is taken by a hakker it will refuse to be taken by other hakkers but the
  // owning hakker can put it back
  when(Taken) {
    case Event(Take, _) =>
...
case object Done extends FSMHakkerState
 ...
 */
class FSMHakker(name: String, left: ActorRef, right: ActorRef, hakker: Hakker)
  ...
  // When a hakker is hungry it tries to pick up its chopsticks and eat. When it picks one up, it
  // goes into wait for the other. If the hakkers first attempt at grabbing a chopstick fails, it
  // starts to wait for the response of the other grab
  when(Hungry) {
  ...
  // When a hakker is waiting for the last chopstick it can either obtain it and start eating, or
  // the other chopstick was busy, and the hakker goes back to think about how he should obtain his
  // chopsticks :-)
  ...
  // When a hakker is eating, he can decide to start to think, then he puts down his chopsticks and
  // starts to think
  when(Eating) {
    case Event(StateTimeout, _) =>
...
object DiningHakkersOnFsm {
  ...
  def run(hakkers: List[Hakker]): ActorSystem = {
    ...
    val chopsticks =
      for (i <- 1 to num) yield system.actorOf(Props(new Chopstick("Chopstick " + i)))

    // Create 5 awesome fsm hakkers and assign them their left and right chopstick
    hakkers.zipWithIndex.map { case (hakker, i) =>
      system.actorOf(Props(
        new FSMHakker(hakker.name, chopsticks((i + 1) % num), chopsticks(i), hakker)))
    }.foreach(_ ! Think)
